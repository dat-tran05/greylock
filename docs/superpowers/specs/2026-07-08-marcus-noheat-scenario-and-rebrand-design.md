...
**Form validation on submit (superseded — now correctness-gated):** `components/TicketForm.tsx` originally blocked submission only when a field was empty. Per explicit follow-up direction, this was tightened to block submission unless *every* field is actually correct (`gradeSubmission(scenario.fields, values).score === scenario.fields.length`), not just non-empty — reusing the same grading function that drives the staff reveal. On any validation failure (incorrect fields or a failed address-realness check), the red outline still goes around the *entire form box*, not the individual offending field, and the message stays generic ("Please review your answers before submitting") — so the error never signals to the visitor which field is wrong or in what way. The correctness check runs before the address-validation network call, so an incorrect submission never reaches the API.

**Design consequence, confirmed intentional:** since submission is now impossible unless every field is correct, the staff reveal's score is always 5/5 and the banner is always APPROVE — DENY is no longer reachable through the normal flow. This also means a visitor can, in principle, brute-force a dropdown by cycling through options until the red box clears, without listening to the roleplay at all. Both consequences were explicitly surfaced and confirmed as acceptable before implementing.

**Address validation hardening:** `lib/address-validation.ts` checks confirmation status on only the *critical* address components — `street_number`, `route`, `locality`, `administrative_area_level_1`, `postal_code` — plus a non-empty `result.address.unresolvedTokens`, in addition to the existing `addressComplete`/`validationGranularity` checks. A component fails the check if it's not `CONFIRMED` or was `inferred` (present in Google's response but not actually supplied in the input).

This was originally a blanket `verdict.hasInferredComponents` check, which turned out to reject nearly every real address: verified live against Google's API, `country` and the ZIP+4 `postal_code_suffix` come back `inferred: true` on virtually every request (nobody types "USA" or a 9-digit ZIP), which tripped the blanket flag even for "2 Jackson Street, San Francisco, CA 94111" — a real, USPS-confirmed business address. Scoping the check to only the 5 critical types fixes that false rejection while still correctly catching the duplicate-street-name case: verified live that "2 Jackson St" alone (no city) resolves to the *same* SF address but with `locality`/`administrative_area_level_1`/`postal_code` all marked `inferred: true` (correctly rejected), and "123 Main Street" alone (a highly duplicated street name) resolves to Vancouver, BC with `street_number` `UNCONFIRMED_BUT_PLAUSIBLE` (correctly rejected).
