...
**Form validation on submit:** `components/TicketForm.tsx` now blocks submission if *any* field is empty (text or dropdown), not just the address — previously a visitor could submit with only the address filled in. On any validation failure (missing fields or a failed address check), the red outline goes around the *entire form box*, not the individual offending field — deliberately, so the error never signals to the visitor which specific field is wrong (that would leak a hint at the puzzle). The generic message ("Please fill in every field before submitting" or the address error) appears below the form and clears as soon as any field is edited. The empty-fields check runs before the address-validation network call, so an incomplete form never reaches the API.
